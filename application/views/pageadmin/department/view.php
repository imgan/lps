                    ...
                    <div class="card card-info">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input required type="text" id="nama" name="nama" class="form-control" placeholder="Nama Department">
                            </div>
                        </div>
                        <!-- /.card-body -->
                    </div>
    ...
    <div class="card">
        ...
        <div class="col-sm-2">
            <button href="#modalTambah" type="button" role="button" data-toggle="modal" class="btn btn-block btn-primary">
                <a class="ace-icon fa fa-plus bigger-120"></a> Add Department</button>
        </div>
        <br>
...
<script type="text/javascript">
    if ($("#formTambah").length > 0) {
        $("#formTambah").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url('administrator/department/simpan') ?>",
                    type: "POST",
    ...
    function show_data() {
        $.ajax({
            type: 'POST',
            url: '<?php echo site_url('administrator/department/tampil') ?>',
            async: true,
            dataType: 'json',
            success: function(data) {
                var html = '';
                ...
                for (i = 0; i < data.length; i++) {
                    ...
                    if (data[i].StatusId == 1) {
                        ...
                        status = '<td class="text-center">' +
                            ...
                            '      <i class="fas fa-times"> </i>  Tidak Aktif </button>' +
                            '</button> &nbsp' +
                            '</td>'
                    }
    ...
    $('#show_data').on('click', '.item_non', function() {
        ...
        Swal.fire({
            title: 'Apakah anda yakin?',
            text: "Anda mengubah status Department menjadi Tidak aktif",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('administrator/department/nonaktif') ?>",
                    async: true,
                    dataType: "JSON",
                    data: {
                        id: id,
