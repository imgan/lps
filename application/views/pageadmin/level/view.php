                    ...
                    <div class="card card-info">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input required type="text" id="nama" name="nama" class="form-control" placeholder="Nama Level">
                            </div>
                        </div>
                        <!-- /.card-body -->
                    </div>
    ...
    <div class="card">
        ...
        <div class="col-sm-2">
            <button href="#modalTambah" type="button" role="button" data-toggle="modal" class="btn btn-block btn-primary">
                <a class="ace-icon fa fa-plus bigger-120"></a> Add Level</button>
        </div>
        <br>
...
<script type="text/javascript">
    if ($("#formTambah").length > 0) {
        $("#formTambah").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url('administrator/level/simpan') ?>",
                    type: "POST",
    ...
    $('#show_data').on('click', '.item_hapus', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('administrator/level/delete') ?>",
                    async: true,
                    dataType: "JSON",
                    data: {
                        id: id,
    ...
    function show_data() {
        $.ajax({
            type: 'POST',
            url: '<?php echo site_url('administrator/level/tampil') ?>',
            async: true,
            dataType: 'json',
            success: function(data) {
                var html = '';
    ...
    $('#show_data').on('click', '.item_edit', function() {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('administrator/level/tampil_byid') ?>",
            async: true,
            dataType: "JSON",
            data: {
                id: id,
    ...
    if ($("#formEdit").length > 0) {
        $("#formEdit").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url('administrator/level/update') ?>",
                    type: "POST",
