<section class="content">
                    ...
                    <div class="card card-info">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <select required class="form-control select2" style="width: 100%;" name="department" id="department">
                ...
                <form class="form-horizontal" role="form" id="formEdit">
                    <div class="card card-info">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                <label>Department</label>
                                <select required class="form-control select2" style="width: 100%;" name="e_department" id="e_department">
                                    <option value="" selected="selected">-- Pilih --</option>
                                    <?php foreach ($mydepartment as $value) { ?>
                                        <option value=<?= $value['Id'] ?>><?= $value['Name']; ?></option>
                                    <?php } ?>
    ...
    <!-- Default box -->
    <div id="modalEwf" class="modal fade" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form class="form-horizontal" role="form" id="formEwf">
                    <div class="card card-info">
                        <div class="modal-header">
                            <h4 class="modal-title">E-Workflow Form</h4>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="card-body">
                            <div class="form-group">
                                <label>Request No</label>
                                <input required  type="hidden" id="e_id" name="e_id" class="form-control" placeholder="ReqId">
                                <input required readonly type="text" id="e_nama2" name="e_nama2" class="form-control" placeholder="Status">
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="form-group">
                                <label>E-workflow Number</label>
                                <input required type="text" id="e_wf" name="e_wf" class="form-control" placeholder="E-Workflow Number">
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
...
<script type="text/javascript">
    ...
    if ($("#formEwf").length > 0) {
        $("#formEwf").validate({
            ...
            submitHandler: function(form) {
                $('#btn_simpan').html('Sending..');
                $.ajax({
                    url: "<?php echo base_url('administrator/request/simpanEwf') ?>",
                    type: "POST",
                    data: $('#formEwf').serialize(),
                    dataType: "json",
                    success: function(response) {
                        $('#btn_simpan').html('<i class="ace-icon fa fa-save"></i>' +
                            'Simpan');
                        if (response == true) {
                            document.getElementById("formEwf").reset();
                            swalInputSuccess();
                            show_data();
                            $('#modalEwf').modal('hide');
                        } else if (response == 401) {
                            ...
                            swalInputFailed("Internal Server error");
                        }
                    }
                });
            }
    ...
    function show_data() {
        $.ajax({
            type: 'POST',
            url: '<?php echo site_url('administrator/request/tampil') ?>',
            async: true,
            dataType: 'json',
            success: function(data) {
                var html = '';
                ...
                for (i = 0; i < data.length; i++) {
                    ...
                    if (data[i].ReqStatus == 1) {
                        ...
                        button = '<td class="project-actions text-left">' +
                            '</td>';
                        status = '<td class="project-state"><span class="badge badge-warning"> Processing.. </span></td>'
                    } else if (data[i].ReqStatus == 3) {
                        button = '<td class="project-actions text-left">' +
                            '   <button  class="btn btn-success btn-sm item_budget"  data-id="' + data[i].ReqId + '">' +
                            '      <i class="fas fa-play"> </i>  Start Budget </a>' +
                            '</td>';
                        status = '<td class="project-state"><span class="badge badge-info"> Find Budget.. </span></td>'
                    } else if (data[i].ReqStatus == 4) {
                        button = '<td class="project-actions text-left">' +
                            '   <button  class="btn btn-danger btn-sm item_stop_budget"  data-id="' + data[i].ReqId + '">' +
                            '      <i class="fas fa-stop"> </i>  Stop Budget </a>' +
                            '</td>';
                        status = '<td class="project-state"><span class="badge badge-warning"> Finding Budget.. </span></td>'
                    } else if (data[i].ReqStatus == 5) {
                        button = '<td class="project-actions text-left">' +
                            '   <button  class="btn btn-success btn-sm item_start_ewf"  data-id="' + data[i].ReqId + '">' +
                            '      <i class="fas fa-play"> </i>  Start E-Workflow </a>' +
                            '</td>';
                        status = '<td class="project-state"><span class="badge badge-info"> E-Workflow .. </span></td>'
                    } else if (data[i].ReqStatus == 6) {
                        button = '<td class="project-actions text-left">' +
                            '   <button  class="btn btn-danger btn-sm item_stop_ewf"  data-id="' + data[i].ReqId + '">' +
                            '      <i class="fas fa-stop"> </i>  Stop E-Workflow </a>' +
                            '</td>';
                        ...
                        button = '<td class="project-actions text-left">' +
                            '</td>';
                        status = '<td class="project-state"><span class="badge badge-info"> Start Buyers .. </span></td>'
                    ...
                    html += '<tr>' +
                        ...
                        '<td class="text-left">' + data[i].ReqNo + '</td>' +
                        '<td class="text-left">' + data[i].ReqDesc + '</td>' +
    ...
    $('#show_data').on('click', '.item_stop_ewf', function() {
        var id = $(this).data('id');
        Swal.fire({
            title: 'Apakah anda yakin?',
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('administrator/request/ewfstop') ?>",
                    async: true,
                    dataType: "JSON",
                    data: {
                        id: id,
    ...
    $('#show_data').on('click', '.item_stop', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('administrator/request/stop') ?>",
                    async: true,
                    dataType: "JSON",
                    data: {
                        id: id,
    ...
    $('#show_data').on('click', '.item_start', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('administrator/request/aktif') ?>",
                    async: true,
                    dataType: "JSON",
                    data: {
                        id: id,
    ...
    $('#show_data').on('click', '.item_budget', function() {
        var id = $(this).data('id');
        Swal.fire({
            title: 'Apakah anda yakin?',
            text: "Anda mengubah Request menjadi aktif",
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('administrator/request/budgetaktif') ?>",
                    async: true,
                    dataType: "JSON",
                    data: {
                        id: id,
    ...
    $('#show_data').on('click', '.item_stop_budget', function() {
        var id = $(this).data('id');
        Swal.fire({
            title: 'Apakah anda yakin?',
            text: "Anda mengubah Request menjadi aktif",
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('administrator/request/budgetstop') ?>",
                    async: true,
                    dataType: "JSON",
                    data: {
                        id: id,
    ...
    $('#show_data').on('click', '.item_start_ewf', function() {
        document.getElementById("formEwf").reset();
        var id = $(this).data('id');
        $('#modalEwf').modal('show');
        $.ajax({
            ...
            success: function(data) {
                $('#e_id').val(data[0].ReqId);
                $('#e_nama2').val(data[0].ReqNo);
