                    ...
                    <div class="card card-info">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <select required class="form-control select2" style="width: 100%;" name="pr" id="pr">
                                    <option value="" selected="selected">-- Pilih --</option>
                                    <?php foreach ($my_data as $value) { ?>
                                        <option value=<?= $value['PrId'] ?>><?= $value['Prno'] ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                            <div class="form-group">
                                <label>PO Number</label>
                                <input required type="text" id="po" name="po" class="form-control" placeholder="PO Number">
                            </div>
    ...
    <div class="card">
        ...
        <div class="col-sm-2">
            <button href="#modalTambah" type="button" role="button" data-toggle="modal" class="btn btn-block btn-primary">
                <a class="ace-icon fa fa-plus bigger-120"></a> Add PO</button>
        </div>
        <br>
...
<script type="text/javascript">
    if ($("#formTambah").length > 0) {
        $("#formTambah").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url('administrator/po/simpan') ?>",
                    type: "POST",
    ...
    $('#show_data').on('click', '.item_hapus', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('administrator/po/delete') ?>",
                    async: true,
                    dataType: "JSON",
                    data: {
                        id: id,
    ...
    function show_data() {
        $.ajax({
            type: 'POST',
            url: '<?php echo site_url('administrator/po/tampil') ?>',
            async: true,
            dataType: 'json',
            success: function(data) {
                var html = '';
                ...
                for (i = 0; i < data.length; i++) {
                    ...
                    if (data[i].ReqStatus == 1) {
                        button = '<td class="text-center">' +
                            '   <button  class="btn btn-danger btn-sm item_non"  data-id="' + data[i].PoId + '">' +
                        ...
                        button = '<td class="text-center">' +
                            '</td>';
                        status = '<td class="project-state"><span class="badge badge-success"> Finish PO.. </span></td>';
                    }
                    ...
                    html += '<tr>' +
                        ...
                        '<td class="text-left">' + data[i].Prnos + '</td>' +
                        '<td class="text-left">' + data[i].PoNo + '</td>' +
    ...
    $('#show_data').on('click', '.item_non', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('administrator/po/stop') ?>",
                    async: true,
                    dataType: "JSON",
                    data: {
                        id: id,
    ...
    $("#pr").change(function() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('administrator/po/showreqno') ?>",
			data: {
				id: id
			}
		}).done(function(data) {
